...
fn parse_kicad_line_to_floats(passed_str: &str) -> (f64,f64){
    ...
    return (x_y_vec[1].parse::<f64>().unwrap(), x_y_vec[2].parse::<f64>().unwrap());
}
pub fn parse_file() -> Placement {
    ...
    while in_doc{ 
        let mut line = content_iter.next().unwrap_or_else(|| {in_doc = false; "bad"});
        if !in_doc{break};
    //for line in contents.split("\n"){
        ...
        if line.starts_with("\t(net ") && line.trim_end().ends_with("\")"){
            ...
            let net_name = words[2].trim_end().strip_suffix(|_: char| true).unwrap().replace("\"", "");
        ...
        if line.starts_with("\t(footprint ") {
            ...
            while !line.contains("pad "){
                line  = content_iter.next().unwrap();
                if line.contains("model")|| line.starts_with("\t)"){break;}
            }
            if line.contains("model") || line.starts_with("\t)"){break;}
            ...
            //println!("{}, {}",, ys[ys.len()-1]);
                ...
                let mut comp: Component = Component{refdes:refdes.to_string(), bbox: comp_bbox, rotation: 0, pins: pin_vec};
